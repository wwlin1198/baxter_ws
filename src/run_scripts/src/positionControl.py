...
# limb is a handle to a limb object
	...
	for i in range(0, frames):
		...
		if jointPos != "invalid":
			# Check if it is minor move. oif it is not, use smoother movement function
			minorMove = True
...
def left_arm_follow_trajectory(fPath, lLimb, pause_event):
...
def right_arm_follow_trajectory(fPath,  rLimb, pause_event):
	# Moves limb to specified joint positions
	# fPath: string indentifying path to file
...
def both_arms_follow_trajectory(lfPath,rfPath, lLimb, rLimb,pause_event):
	# Moves limb to specified joint positions
	# fPath: string indentifying path to file
	# lLimb: handle to the left limb 'Limb' object
	...
	lwpArray = json.loads(lfText)
	rwpArray = json.loads(rfText)

	LinitPoint = json.loads(json.dumps(lLimb.joint_angles()))
	RinitPoint = json.loads(json.dumps(rLimb.joint_angles()))

	lwpArray.insert(0, LinitPoint)
	rwpArray.insert(0, RinitPoint)

	lpath, lperiod = splineWaypoints(lwpArray)
	rpath, rperiod = splineWaypoints(rwpArray)
	...
	if len(lpath) > len(rpath):
		length = len(lpath)
		rate = rospy.Rate(1/lperiod)
	else:
		length = len(rpath)
		rate = rospy.Rate(1/rperiod)

	for i in range(length):
		waitForNotPause(pause_event)
		if i < len(lpath):
			if lpath[i] != "invalid":
				lLimb.set_joint_positions(lpath[i])

		if i < len(rpath):
			if rpath[i] != "invalid":
				rLimb.set_joint_positions(rpath[i])
		rate.sleep()
